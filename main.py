...
class ResizerApp(MDApp):
    def build(self):
        ...
        self.path = MDTextField(hint_text="Masukkan nama file (contoh: foto.jpg)")
        layout.add_widget(self.path)

        self.width_in = MDTextField(hint_text="Lebar (px)", input_filter="int")
        layout.add_widget(self.width_in)

        self.height_in = MDTextField(hint_text="Tinggi (px)", input_filter="int")
    ...
    def ubah_foto(self, *args):
        try:
            ...
            self.path.text = "Berhasil! Cek folder aplikasi."
        except:
            self.path.text = "Error: File tidak ditemukan!"
